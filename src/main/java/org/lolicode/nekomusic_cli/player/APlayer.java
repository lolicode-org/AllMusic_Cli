package org.lolicode.nekomusic_cli.player;
...
import org.lolicode.nekomusic_cli.player.decoder.flac.FlacDecoder;
import org.lolicode.nekomusic_cli.player.decoder.mp3.Mp3Decoder;
import org.lolicode.nekomusic_cli.player.decoder.ogg.OggDecoder;
...
public class APlayer extends InputStream {
    ...
    public APlayer() {
        try {
            new Thread(this::run, "nekomusic_run").start();
        ...
        while (true) {
            try {
                ...
                try {
                    ...
                    e.printStackTrace();
                    NekoMusic_Cli.sendMessage("[NekoMusic客户端]获取音乐失败");
                    continue;
                }
                ...
                if (!decoder.set()) {
                    ...
                    if (!decoder.set()) {
                        ...
                        if (!decoder.set()) {
                            NekoMusic_Cli.sendMessage("[NekoMusic客户端]不支持这样的文件播放");
                            continue;
                        }
                    }
                }
                ...
                while (!isClose && AL10.alGetSourcei(index,
                    ...
                    AL10.alSourcef(index, AL10.AL_GAIN, NekoMusic_Cli.getVolume());
                    Thread.sleep(100);
                }
                if (!reload) {
                    wait = true;
